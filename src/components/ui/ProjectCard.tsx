...
export function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <TiltCard className="w-full rounded-3xl" maxTilt={6} scale={1.015} enableGlare={true}>
      <div className="group bg-white/70 hover:bg-white border border-border-gray rounded-3xl p-8 md:p-12 transition-all duration-300 shadow-sm hover:shadow-md grid grid-cols-1 lg:grid-cols-4 gap-8 w-full h-full transform-gpu [transform-style:preserve-3d]">
        {/* Info Column */}
        <div className="lg:col-span-3 flex flex-col gap-6 [transform-style:preserve-3d]">
          <div className="flex items-center gap-4 [transform-style:preserve-3d]">
            <div 
              style={{ transform: "translateZ(30px)" }}
              className="w-12 h-12 rounded-full border border-border-gray flex items-center justify-center font-bebas text-xl font-bold text-accent-red group-hover:bg-accent-red group-hover:text-white transition-colors duration-300 shrink-0 transform-gpu"
            >
              {project.num}
            </div>
          ...
          <div className="flex flex-col gap-2 [transform-style:preserve-3d]" style={{ transform: "translateZ(20px)" }}>
          ...
          <div 
            ...
            className="flex items-start gap-3 bg-[#fbfbfa] p-4 rounded-xl border border-border-gray/50 max-w-2xl"
          >
        ...
        {/* Role & Link Column */}
        <div className="border-t lg:border-t-0 lg:border-l border-border-gray pt-6 lg:pt-0 lg:pl-8 flex flex-col justify-between items-start lg:items-end gap-6 [transform-style:preserve-3d]" style={{ transform: "translateZ(25px)" }}>
